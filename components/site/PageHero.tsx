...
export async function PageHero({
  eyebrow, title, subtitle, image, script, heroKey,
}: {
  eyebrow: string;
  title: string;
  subtitle?: string;
  image: string;
  script?: string;
  heroKey?: string; // resolves heroes/<key>.* from Storage, else uses `image`
}) {
  const src = heroKey ? await heroFor(heroKey, image) : image;
  return (
    <section className="relative flex min-h-[62vh] items-center justify-center overflow-hidden pt-24">
      <Image src={src} alt="" fill priority className="animate-zoom object-cover" sizes="100vw" />
