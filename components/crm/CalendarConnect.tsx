  ...
  return (
    <Panel title={<span className="flex items-center gap-2"><CalendarSync size={16} className="text-brass" /> External calendars & OTA sync</span>}>
      ...
      <div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
        {providers.map((p) => (
          <a key={p.key} href={p.method === "OAuth" ? `/api/integrations/calendar/callback?provider=${p.key}` : "#ical-import"}
      ...
      <div id="ical-import" className="mt-5 rounded-xl bg-bone p-4">
        ...
        <div className="mt-2 flex flex-wrap gap-2">
          <input value={url} onChange={(e) => setUrl(e.target.value)} placeholder="https://…/calendar.ics" className="min-w-[220px] flex-1 rounded-lg border border-ink/12 bg-parchment px-3 py-2 text-sm outline-none focus:border-brass" />
          <select value={slug} onChange={(e) => setSlug(e.target.value)} className="rounded-lg border border-ink/12 bg-parchment px-3 py-2 text-sm outline-none focus:border-brass">
