            ...
            <div className="flex flex-1 flex-col gap-2.5">
              {colItems.map((l) => (
                <article
                  ...
                  className={`cursor-grab rounded-xl bg-parchment p-3.5 shadow-sm ring-1 ring-ink/5 transition active:cursor-grabbing ${
                    dragId === l.id ? "opacity-50" : "hover:shadow-md"
                  }`}
                >
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0">
                      <p className="truncate text-sm font-medium text-ink">{l.name}</p>
                      <p className="text-xs text-stone">{l.eventType} · {formatDate(l.eventDate)}</p>
                    </div>
